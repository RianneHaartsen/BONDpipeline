%% LEAP pipelines during soc-nsoc videos: get power and connectivity values from BOND pipeline data

% This script takes the data processed with the BONDpipeline and converges
% it to a fieldtrip structure. Then power is calculated
% for 1 to 32 Hz for each trial. Finally, data are split
% into conditions and the dbWPLI is calculated for the social condition,
% non-social condition, and across all conditions. 
% Processing is looped through participants with paths from the
% BOND_preprocessing_report.mat file.

% This scripts further depends on EEGlab and fieldtrip functions. 
% The eeglab2fieldtrip function can be downloaded from
% https://github.com/fieldtrip/fieldtrip/blob/master/external/eeglab/eeglab2fieldtrip.m
% .

% Same scripts were run on the pre-processed data from the manual, MADE,
% and HAPPE v1 pipeline. 

% Rianne Haartsen, PhD.; 2022 
...
ft_defaults
addpath(genpath('xxx')) % any additional folders needed. 

load /xxx/BOND_preprocessing_report.mat % resulting report from the BOND pipeline with paths to pre-processed

Pow_location = 'xxx/A_Power_data'; % folder where individual power data will be stored
FC_location = 'xxx/B_FC_data'; % folder where individual connectivity will be stored
output_location = 'xxx'; % location where the file with metrics of interest will be stored
Fig_location = 'xxx/Report_figs'; % location where report figures will be stored (spectra and topoplots for theta and alpha frequency bands) 


for subject = 1:height(BOND_report_table) 
    
    CurrID = BOND_report_table.ID{subject};
    
    fprintf('\n\n\n*** Processing subject %d (%s) ***\n\n\n', subject, CurrID);
    
    try
        % load preprocessed data
        filename_path_preproc = BOND_report_table.path_preproc{subject};
    ...
    BOND_MOI_table(subject,:) = data_table_newrow;
    save('BOND_MeasuresOfInterest_report.mat','BOND_MOI_table');
...
%% Quick check to look at individual power/FC spectra for each condition and across conditions 
figure; plot(BOND_MOI_table.Freqs{1,1}',cell2mat(BOND_MOI_table.GlobPow_all)')
figure; plot(BOND_MOI_table.Freqs{1,1}',cell2mat(BOND_MOI_table.GlobPow_soc)')
figure; plot(BOND_MOI_table.Freqs{1,1}',cell2mat(BOND_MOI_table.GlobPow_toy)')
figure; plot(BOND_MOI_table.Freqs{1,1}',cell2mat(BOND_MOI_table.GlobFC_all)')
figure; plot(BOND_MOI_table.Freqs{1,1}',cell2mat(BOND_MOI_table.GlobFC_soc)')
figure; plot(BOND_MOI_table.Freqs{1,1}',cell2mat(BOND_MOI_table.GlobFC_toy)')
