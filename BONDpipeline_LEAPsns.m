%% Analysis of social and non-social videos in LEAP; v2
% using the BOND pipeline (MADE pipeline with minor adjustments)

% Rianne Haartsen, PhD.; 09-2022 
% Birkbeck University of London

% Adjustments made relative to original MADE pipeline:
% - adjustment for flat channels
% - tracking n channels for each stage of preprocessing


% Note; folders commented out where appropriate for sharing on github
% additional comments by Rianne Haartsen are noted with '%RH;'
...
TimingsDatapath = 'xxx'; % RH; this variable is specific to the LEAP social/non-social videos dataset only
...
lowpass  = 32; % Low-pass frequency. We recommend low-pass filter at/below line noise frequency (see manuscript for detail)
    % RH: we use 1 to 32Hz here for fair comparison to the manual pipeline

% 8. Are you processing task-related or resting-state EEG data?
task_eeg = 2; % 0 = resting, 1 = task, 2 = resting state with 2 conditions
...
frontal_channels = {'F8', 'AF8', ' Fp2', 'Fp1', 'AF7', 'F7'}; % If you set interp_epoch = 1, enter the list of frontal channels to check (see manuscript for detail)
% recommended list for EGI 128 channel net: {'E1', 'E8', 'E14', 'E21', 'E25', 'E32', 'E17'}
% equivalent in LEAP EEG layout: {'F8', 'AF8', ' Fp2', 'Fp1', 'AF7', 'F7'} 
...
for subject= 1:length(datafile_names)
    ...
    try
        ...
        load xxx/LEAPCommonChannels.mat % RH; this is specific to the LEAP social/non-social videos - we decided to include common channels across all sites only
        EEG = pop_select(EEG, 'nochannel', find(~ismember(lower({EEG.chanlocs.labels}),lower(LEAPCommonChannels)))); % keep only channels that correspond to the 59 core channels 
        EEG = eeg_checkset(EEG);

        % LEAP site
        SiteCur = EEG.euaims.site; % RH; specific to the LEAP social/non-social videos 
        % comments
        ExpComments = EEG.euaims.comments; % RH; specific to the LEAP social/non-social videos 
    ...
    %    edit_event_markers_example() % an example of a labeling script from the appendix folder that can be called
        ...
        % resample to 1000Hz if srate is not 1000Hz to make sure all are
        ...
        %% Step 6 1/2: Remove non-SNSvid data in LEAP recordings and check the timing
        % RH; This step was added to ensure the timing in the EEG is
        % consistent with the timing externally recorded by the task tracker. 
            ...
            samples_per_video = rest_video_length * EEG.srate;
             ...
             % Check whether the end timing is after the time end of last video
             Subj = extractBefore(datafile_names{subject},'_social_nonsocial_videos.set');
                Flash_name = fullfile(TimingsDatapath,sprintf('Tablesocialnonsocial - %i.mat',str2double(Subj)));
                    ...
                    if isequal(size(VidLength,1),size(segment_boundaries,1))
                        ...
                        for tt = 1:size(VidLength,1)
                            if VidLength(tt,1) < rest_video_length % check if flash before 60sec after video onset
                                disp('Timing before end of video; correcting now')
                ...
                elseif strcmpi(SiteCur,'UMCU') %compare with the video offset event
                    ...
                    warning('No external timing information found, skipping this step')
                    FlashCheck = 0;
                end
                 TimingChecks(subject) = FlashCheck; clear FlashCheck
        ...
        if all_chan_bad_Flat==1 % set rest of report variables to 0
            ...
            report_table_newrow = table({ID}, datafile_name_preproc_done(1,subject), preproc_path(1,subject), date_preproc(1,subject), ...
                {SiteCur}, {ExpComments}, {TimingChecks(1,subject)}, ...
...
%             end
        ...
        if epoch_data==1
            ...
            elseif task_eeg==2 % resting eeg - 2 condition
                ...
                % RH; we opted for the resting state EEG here as this is a
                % similar design with continous data collection in 2
                % conditions as our social/non-social videos. 
        ...
        if ~isempty(error_log)
            ...
            if exist('BOND_preprocessing_error_log.mat','file')
                load BOND_preprocessing_error_log.mat
            end
            BONDpreproc_error_log{subject} = error_log;
            save('BONDpreproc_error_log.mat','BONDpreproc_error_log')
        end
        % update other tracking variables
        datafile_name_preproc_done{subject} = 'NA';
        preproc_path{subject} = 'NA';
        date_preproc{subject} = datestr(now,'dd-mm-yyyy');
        if length(TimingChecks) < subject || isempty(TimingChecks(subject))
            TimingChecks(subject) = NaN;
        end
